...
app.post('/register', async (req, res) => {
  ...
  const token = jwt.sign({ id: data[0].id, name: data[0].name }, process.env.JWT_SECRET);
  res.json({ token, name: data[0].name, id: data[0].id });
});
...
app.post('/save-list', async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      await supabase.from('wishlist').update({ items }).eq('id', existing.id);
    } else {
      await supabase.from('wishlist').insert([{ user_id: decoded.id, season, items }]);
    }
